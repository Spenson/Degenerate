...
extern glm::mat4 AIMatrixToGLMMatrix(const aiMatrix4x4& mat);
//{
...
cSimpleSkinnedMesh::cSimpleSkinnedMesh(void)
{
	this->pScene = 0;
	this->mNumBones = 0;
...
void cSimpleSkinnedMesh::ShutErDown(void)
{
...
bool cSimpleSkinnedMesh::LoadMeshFromFile(const std::string& friendlyName,
												const std::string& filename)		// mesh we draw
{
	...
	//aiMesh* pM0 = this->mScene->mMeshes[0];
...
float cSimpleSkinnedMesh::FindAnimationTotalTime(std::string animationName)
{
...
bool cSimpleSkinnedMesh::LoadMeshAnimation(const std::string& friendlyName,
...
void cSimpleSkinnedMesh::GetListOfBoneIDandNames(std::vector<std::string>& vecBoneNames)
{
...
//const aiScene* mScene;		// "pretty" mesh we update and draw
...
bool cSimpleSkinnedMesh::Initialize(void)
{
...
void cSimpleSkinnedMesh::sVertexBoneData::AddBoneData(unsigned int BoneID, float Weight)
{
...
//void cSimpleSkinnedMesh::BoneTransform( float TimeInSeconds, 
...
void cSimpleSkinnedMesh::LoadBones(const aiMesh* Mesh, std::vector<sVertexBoneData>& vertexBoneData)
{
...
const aiNodeAnim* cSimpleSkinnedMesh::FindNodeAnimationChannel(const aiAnimation* pAnimation, aiString boneName)
{
...
unsigned int cSimpleSkinnedMesh::FindRotation(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int cSimpleSkinnedMesh::FindPosition(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int cSimpleSkinnedMesh::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void cSimpleSkinnedMesh::ReadNodeHeirarchy(float AnimationTime,
...
void cSimpleSkinnedMesh::CalcInterpolatedRotation(float AnimationTime, const aiNodeAnim* pNodeAnim, aiQuaternion& out)
{
...
void cSimpleSkinnedMesh::CalcInterpolatedPosition(float AnimationTime, const aiNodeAnim* pNodeAnim, aiVector3D& out)
{
...
void cSimpleSkinnedMesh::CalcInterpolatedScaling(float AnimationTime, const aiNodeAnim* pNodeAnim, aiVector3D& out)
{
...
void cSimpleSkinnedMesh::CalcGLMInterpolatedRotation(float AnimationTime, const aiNodeAnim* pNodeAnim, glm::quat& out)
{
	if (pNodeAnim->mNumRotationKeys == 1)
	{
...
void cSimpleSkinnedMesh::CalcGLMInterpolatedPosition(float AnimationTime, const aiNodeAnim* pNodeAnim, glm::vec3& out)
{
	if (pNodeAnim->mNumPositionKeys == 1)
	{
...
void cSimpleSkinnedMesh::CalcGLMInterpolatedScaling(float AnimationTime, const aiNodeAnim* pNodeAnim, glm::vec3& out)
{
	if (pNodeAnim->mNumScalingKeys == 1)
	{
...
float cSimpleSkinnedMesh::GetDuration(void)
{
...
//cMesh* cSimpleSkinnedMesh::CreateMeshObjectFromCurrentModel( unsigned int meshIndex /*=0*/)
//
// Returns NULL (0) if there is no mesh at that index
sModelDrawInfo* cSimpleSkinnedMesh::CreateModelDrawInfoObjectFromCurrentModel(unsigned int meshIndex/*=0*/)
{
...
//	cMesh* pTheMesh = new cMesh();
	...
	pTheMDI->pVertices = new sComplexVertex[pTheMDI->numberOfVertices];

	for (int vertIndex = 0; vertIndex != pTheMDI->numberOfVertices; vertIndex++)
	{
		sComplexVertex* pCurVert = &(pTheMDI->pVertices[vertIndex]);
