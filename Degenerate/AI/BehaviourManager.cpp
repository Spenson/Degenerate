...
namespace DegenBehaviours
{
	...
	{
		// Perform cleanup
		for (behaviour_iterator itAgent = mBehaviourMap.begin(); itAgent != mBehaviourMap.end(); ++itAgent)
		{
			if (itAgent->second != 0)
			{
	...
	void BehaviourManager::SetBehaviour(iEntity* agent, iBehaviour* behaviour)
	{
