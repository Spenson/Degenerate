...
std::map<std::string, cSkinnedMesh::SceneImporter> cSkinnedMesh::FullScenes;
std::map<std::string, cSkinnedMesh::SceneImporter> cSkinnedMesh::AnimationOnly;
...
bool cSkinnedMesh::Initialize()
{
	this->m_numberOfVertices = this->mScene->mMeshes[mMeshIndex]->mNumVertices;

	// This is the vertex information for JUST the bone stuff
	this->vecVertexBoneData.resize(this->m_numberOfVertices);

	this->LoadBones(this->mScene->mMeshes[mMeshIndex], this->vecVertexBoneData);
...
bool cSkinnedMesh::LoadMeshFromFile(const std::string& friendlyName, const std::string& filename, const unsigned int meshIndex)
{

	if (FullScenes.find(filename) == FullScenes.end())
	{
	unsigned int Flags = aiProcess_Triangulate |
		...
		FullScenes[filename].Scene = FullScenes[filename].Importer.ReadFile(filename.c_str(), Flags);
		if (!FullScenes[filename].Scene)
		{
			FullScenes.erase(filename);
			return false;
		}
	}

	this->mScene = FullScenes[filename].Scene;
...
bool cSkinnedMesh::LoadMeshAnimation(const std::string& friendlyName, const std::string& filename, const unsigned int aniIndex)
{
	// Already loaded this? 
	...
	animInfo.animationIndex = aniIndex;

	if (FullScenes.find(filename) != FullScenes.end())
	{
		animInfo.pAIScene = FullScenes[filename].Scene;
	}
	else
	{
		if (AnimationOnly.find(filename) == AnimationOnly.end())
		{
			...
			AnimationOnly[filename].Scene = AnimationOnly[filename].Importer.ReadFile(animInfo.fileName.c_str(), Flags);
			if (!AnimationOnly[filename].Scene)
			{
				AnimationOnly.erase(filename);
				return false;
			}
		}
		animInfo.pAIScene = AnimationOnly[filename].Scene;
	}
	...
	animInfo.animationTime = animInfo.pAIScene->mAnimations[aniIndex]->mDuration / this->mScene->mAnimations[0]->mTicksPerSecond;
...
sModelDrawInfo* cSkinnedMesh::CreateModelDrawInfoObjectFromCurrentModel()
{
	sModelDrawInfo* pTheMDI = new sModelDrawInfo();

	aiMesh* pAIMesh = this->mScene->mMeshes[mMeshIndex];
...
void cSkinnedMesh::BoneTransform(float TimeInSeconds, std::string animationName,
								 ...
								 std::vector<glm::mat4>& Offsets)
{
	...
	float TicksPerSecond = static_cast<float>(this->mScene->mAnimations[0]->mTicksPerSecond != 0 ?
											  this->mScene->mAnimations[0]->mTicksPerSecond : 25.0);

	float TimeInTicks = TimeInSeconds * TicksPerSecond;
	
	//float AnimationTime = fmod(TimeInTicks, (float)this->mScene->mAnimations[0]->mDuration);
	float AnimationTime = fmod(TimeInTicks, (float)mapAnimationFriendlyNameTo_pScene[animationName].pAIScene->mAnimations[mapAnimationFriendlyNameTo_pScene[animationName].animationIndex]->mDuration);
...
void cSkinnedMesh::ReadNodeHeirarchy(float AnimationTime, std::string animationName, const aiNode* pNode, const glm::mat4& parentTransformMatrix)
{
...
// Did we find it? 
	if (itAnimation != this->mapAnimationFriendlyNameTo_pScene.end())
	{
		...
		pAnimation = reinterpret_cast<const aiAnimation*>(itAnimation->second.pAIScene->mAnimations[itAnimation->second.animationIndex]);
	}
...
{
	float duration = (float)(this->mScene->mAnimations[0]->mDuration / this->mScene->mAnimations[0]->mTicksPerSecond);
