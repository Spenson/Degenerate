...
void main()
{
	if (passNumber == 1)
	{
		...
		if (blur)
		{
			...
			vec3 texRGB1 = texture(texture00, vec2(uvs.s + 0.0f, uvs.t + 0.0f)).rgb;

			vec3 texRGB2 = texture(texture00, vec2(uvs.s - bo, uvs.t + 0.0f)).rgb;
			vec3 texRGB3 = texture(texture00, vec2(uvs.s + bo, uvs.t + 0.0f)).rgb;
			vec3 texRGB4 = texture(texture00, vec2(uvs.s + 0.0f, uvs.t - bo)).rgb;
			vec3 texRGB5 = texture(texture00, vec2(uvs.s + 0.0f, uvs.t + bo)).rgb;

			pixelColour.rgb = 0.5f * texRGB1 +
				0.125f * texRGB2 +
				0.125f * texRGB3 +
				0.125f * texRGB4 +
				0.125f * texRGB5;
		...
		{
			vec3 tex0_RGB = texture(texture00, uvs.st).rgb;
			vec3 tex1_RGB = texture(texture01, uvs.st).rgb;
			vec3 tex2_RGB = texture(texture02, uvs.st).rgb;
			vec3 tex3_RGB = texture(texture03, uvs.st).rgb;

			pixelColour.rgb = (texture_ratios.x * tex0_RGB)
