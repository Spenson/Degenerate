...
float LightHelper::calcApproxDistFromAtten(float targetLightLevel)	// Uses the defaults
{
	return this->calcApproxDistFromAtten(targetLightLevel, LightHelper::DEFAULDIFFUSEACCURACYTHRESHOLD);
}

float LightHelper::calcApproxDistFromAtten(float targetLightLevel, float accuracy)	// Uses the defaults
{
											...
											unsigned int maxIterations /*= DEFAULTMAXITERATIONS = 50*/)
{
	...
	if (this->calcDiffuseFromAttenByDistance(LightHelper::DEFAULTINFINITEDISTANCE,
